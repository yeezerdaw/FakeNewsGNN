...
torch.backends.cudnn.benchmark = False  # Fixed Typo
...
def train_model(model, x, edge_index, y, epochs=1000, lr=0.01):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=1e-5)  # Added L2 Regularization
    ...
    model.eval()
    with torch.no_grad():
        train_predictions = model(data.x).argmax(dim=1)
